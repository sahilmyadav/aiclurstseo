...
import { FiAlertTriangle, FiCheck, FiMinus, FiPlus, FiStar, FiUsers, FiX } from 'react-icons/fi';
...
const SubscriptionPage = () => {
  ...
  if (authLoading || !user) {
    return (
      <div
        className={`min-h-screen flex items-center justify-center ${theme === 'dark' ? 'bg-gradient-to-br from-gray-900 via-gray-800 to-black' : 'bg-gray-50'}`}
      >
  ...
  const handleSubscribe = async (planType) => {
    try {
      ...
      const response = await axios.post(
        `${getApiBaseUrl()}/api/subscription/create-checkout-session`,
        requestData,
        {
          headers: {
  ...
  return (
    <div
      className={`min-h-screen py-8 ${theme === 'dark' ? 'bg-gradient-to-br from-gray-900 via-gray-800 to-black' : 'bg-[radial-gradient(at_40%_20%,hsl(250,91%,97%)_0px,transparent_50%),radial-gradient(at_80%_0%,hsl(340,82%,97%)_0px,transparent_50%),radial-gradient(at_0%_50%,hsl(160,84%,97%)_0px,transparent_50%)]'}`}
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        {/* Header */}
        <div className="text-center mb-6 px-2 sm:px-0">
          <h1
            className={`text-2xl md:text-3xl font-bold mb-2 ${theme === 'dark' ? 'bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent' : 'text-purple-600'}`}
          >
            Choose Your Plan
          </h1>
          <p
        ...
        <div className="px-2 sm:px-0">
          {(() => {
            ...
            if (subscriptionData?.status === 'active' && trialData && remainingDays > 0) {
              return (
                <div
                  className={`rounded-2xl shadow-lg p-6 mb-8 ${theme === 'dark' ? 'bg-gray-800 border border-green-900/50' : 'bg-white border border-green-200'}`}
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="text-right">
                      <div
                        className={`text-3xl font-bold ${theme === 'dark' ? 'bg-gradient-to-r from-green-400 to-emerald-400 bg-clip-text text-transparent' : 'text-purple-600'}`}
                      >
                        {remainingDays}
                      </div>
                      <div
                  ...
                  <div
                    className={`rounded-full h-3 ${theme === 'dark' ? 'bg-gradient-to-r from-gray-700 to-gray-600' : 'bg-gray-200'}`}
                  >
                    <div
            ...
            if (trialEligible && (!subscriptionData || !subscriptionData.planType)) {
              return (
                <div
                  className={`rounded-2xl shadow-xl p-6 mb-8 relative overflow-hidden ${theme === 'dark' ? 'bg-gradient-to-r from-blue-600 via-purple-600 to-blue-700 text-white' : 'bg-gradient-to-r from-purple-500 to-indigo-600 text-white'}`}
                >
                  <div
                    className={`absolute inset-0 ${theme === 'dark' ? 'bg-gradient-to-r from-blue-600/20 to-purple-600/20 backdrop-blur-sm' : 'bg-gradient-to-r from-purple-500/20 to-indigo-600/20 backdrop-blur-sm'}`}
                  ></div>
                  <div className="relative flex items-center justify-between">
                    <div>
                      ...
                      <p
                        className={`mt-1 ${theme === 'dark' ? 'text-blue-100' : 'text-purple-100'}`}
                      >
            ...
            if (subscriptionData?.planType === 'trial' && remainingDays <= 0) {
              return (
                <div
                  className={`rounded-2xl shadow-lg p-6 mb-8 ${theme === 'dark' ? 'bg-gray-800 border border-red-900/50' : 'bg-white border border-red-200'}`}
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className={`mt-1 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                        Your 14-day free trial has ended. Choose a plan to continue enjoying premium
                        features.
                      </p>
                    </div>
                    <div className="text-right">
                      <div
                        className={`text-2xl font-bold ${theme === 'dark' ? 'text-red-400' : 'text-red-500'}`}
                      >
                        Expired
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      >
                        ...
                        {subscriptionData.planType
                          ? `${subscriptionData.planType.charAt(0).toUpperCase() + subscriptionData.planType.slice(1)} Plan Active`
                          : 'Subscription Active'}
                      </h3>
                      <p className={`mt-1 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
        ...
        {/* Profile Selector */}
        <div
          className={`rounded-2xl shadow-lg p-6 mb-8 mx-2 sm:mx-0 ${theme === 'dark' ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}
        >
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center mb-3 sm:mb-0">
              ...
              <div className="hidden sm:block w-12 h-12 rounded-xl mr-4 flex-shrink-0">
                <div
                  className={`w-full h-full rounded-xl flex items-center justify-center ${theme === 'dark' ? 'bg-gradient-to-r from-blue-500 to-purple-500' : 'bg-gradient-to-r from-purple-500 to-indigo-500'}`}
                >
              ...
              <div>
                ...
                <div className="sm:hidden w-16 h-10 rounded-xl flex items-center justify-center mt-2 ${theme === 'dark' ? 'bg-gradient-to-r from-blue-900/50 to-purple-900/50' : 'bg-purple-100'}">
                  <span
                    className={`text-xl font-bold ${theme === 'dark' ? 'bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent' : 'text-purple-600'}`}
                  >
                    {profiles}
                  </span>
                </div>
        ...
        {plansLoading && (
          <div
            className={`rounded-2xl shadow-lg p-4 mb-4 text-center ${theme === 'dark' ? 'bg-gray-800 border border-gray-700 text-gray-300' : 'bg-white border border-gray-200 text-gray-600'}`}
          >
            Loading plans...
          </div>
        )}
        {plansError && (
          <div
            className={`rounded-2xl p-4 mb-4 text-center text-sm ${theme === 'dark' ? 'bg-red-900/60 border border-red-600 text-red-200' : 'bg-red-100 border border-red-300 text-red-700'}`}
          >
            {plansError}
          </div>
        )}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 px-2 sm:px-0">
          ...
          {getPlanByType('daily') && (
            <div
              className={`rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 relative overflow-hidden ${theme === 'dark' ? `bg-gray-800 border ${getPlanByType('daily').isPopular ? 'border-blue-500 border-2' : 'border-gray-700'}` : `bg-white border ${getPlanByType('daily').isPopular ? 'border-purple-500 border-2' : 'border-gray-200'}`}`}
            >
              {/* Popular Badge */}
              {getPlanByType('daily').isPopular && (
                <div
                  className={`absolute top-0 right-0 text-xs font-bold px-3 py-1 rounded-bl-lg ${theme === 'dark' ? 'bg-blue-600 text-white' : 'bg-purple-600 text-white'}`}
                >
              ...
              {getPlanByType('daily').hasDiscount && (
                <div
                  className={`absolute top-0 left-0 text-xs font-bold px-3 py-1 rounded-br-lg ${theme === 'dark' ? 'bg-emerald-600 text-white' : 'bg-green-600 text-white'}`}
                >
              ...
              <div className="p-6">
                ...
                <div className="my-6">
                  {getPlanByType('daily').hasDiscount && (
                    <span
                      className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'} line-through`}
                    >
                      ${(getPlanByType('daily').originalPricePerProfile * profiles).toFixed(2)}
                    </span>
                  )}
                  <div
                    className={`text-3xl font-bold my-2 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}
                  >
                  ...
                  {getPlanByType('daily').hasDiscount && (
                    <span
                      className={`text-sm ${theme === 'dark' ? 'text-emerald-400' : 'text-green-600'}`}
                    >
                      Save {getPlanByType('daily').discountPercent}%
                    </span>
                  )}
                ...
                <div className="space-y-3 mb-6">
                  {(getPlanByType('daily').features || []).map((feature, index) => (
                    <div key={index} className="flex items-center">
                      <div
                        className={`w-5 h-5 rounded-full flex items-center justify-center mr-3 ${theme === 'dark' ? 'bg-blue-500' : 'bg-purple-500'}`}
                      >
                ...
                {getPlanByType('daily').isActive && (
                  <div
                    className={`absolute top-2 right-2 text-xs font-bold px-2 py-1 rounded ${theme === 'dark' ? 'bg-green-600 text-white' : 'bg-green-500 text-white'}`}
                  >
                    ACTIVE
                  </div>
                )}
          ...
          {getPlanByType('monthly') && (
            <div
              className={`rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 relative overflow-hidden ${theme === 'dark' ? `bg-gray-800 border ${getPlanByType('monthly').isPopular ? 'border-blue-500 border-2' : 'border-gray-700'}` : `bg-white border ${getPlanByType('monthly').isPopular ? 'border-purple-500 border-2' : 'border-gray-200'}`}`}
            >
              {/* Popular Badge */}
          ...
          {getPlanByType('yearly') && (
            <div
              className={`rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 relative overflow-hidden ${theme === 'dark' ? `bg-gray-800 border ${getPlanByType('yearly').isPopular ? 'border-blue-500 border-2' : 'border-gray-700'}` : `bg-white border ${getPlanByType('yearly').isPopular ? 'border-purple-500 border-2' : 'border-gray-200'}`}`}
            >
              {/* Popular Badge */}
              ...
              <div className="p-6">
                ...
                >
                  {yearlyLoading
                    ...
                    : getPlanByType('yearly').isActive
                      ? 'Current Plan'
                      : getPlanByType('yearly')?.buttonText ||
                        `Save ${getPlanByType('yearly')?.discountPercent}%`}
                </button>
              </div>
            </div>
        ...
        <div className="text-center mt-8">
          <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
            ...
            <a
              ...
              className={`hover:underline ${theme === 'dark' ? 'text-blue-400 hover:text-blue-300' : 'text-purple-600 hover:text-purple-700'}`}
            >
              Contact Support
            </a>
          </p>
      ...
      {showSubscriptionDialog && (
        <div
          className={`fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 p-4 ${theme === 'dark' ? 'bg-opacity-70' : 'bg-opacity-50'}`}
        >
          <div
            className={`rounded-2xl max-w-md w-full p-6 relative ${theme === 'dark' ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}
          >
            <button
            ...
            <div className="flex items-start mb-6">
              <div
                className={`p-3 rounded-full mr-4 ${theme === 'dark' ? 'bg-yellow-500/20' : 'bg-yellow-100'}`}
              >
                <FiAlertTriangle
                  className={`w-6 h-6 ${theme === 'dark' ? 'text-yellow-500' : 'text-yellow-600'}`}
                />
              </div>
              <div>
                <h3
            ...
            <div
              className={`rounded-xl p-4 mb-6 ${theme === 'dark' ? 'bg-gray-700/50' : 'bg-gray-100'}`}
            >
              <h4
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${theme === 'dark' ? 'text-green-400' : 'text-green-600'}`}
                  >
                    Active
                  </span>
                </div>
            ...
            <div
              className={`rounded-lg p-4 mb-6 ${theme === 'dark' ? 'bg-blue-900/20 border border-blue-800/50' : 'bg-purple-100 border border-purple-200'}`}
            >
              <p className={`text-sm ${theme === 'dark' ? 'text-blue-300' : 'text-purple-700'}`}>
                You cannot purchase a new subscription while you have an active plan. Please wait
                until your current plan expires on{' '}
                {subscriptionData?.endDate
                  ? formatDate(subscriptionData.endDate)
                  : 'the expiry date'}
                .
