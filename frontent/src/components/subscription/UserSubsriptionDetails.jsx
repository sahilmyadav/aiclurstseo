  ...
  useEffect(() => {
    ...
    const fetchBillingData = async () => {
      try {
        ...
        // Use dynamic API base URL (supports IP-based hosting)
        const baseURL = getApiBaseUrl();

        // Fetch user data (either current user or specified user for admin)
        try {
          const userResponse = await axios.get(`${baseURL}/api/auth/user/${targetUserId}`, {
            headers: {
        ...
        try {
          ...
          if (subscriptionError.response) {
            console.error(
              ...
              subscriptionError.response.status,
        ...
        try {
          ...
          if (transactionError.response) {
            console.error(
              ...
              transactionError.response.status,
            ...
            {subscriptionData ? (
              <div className="mt-4 space-y-4">
                <div className="text-center">
                  ...
                  <p className="text-xl font-bold text-white">{subscriptionData.planType} Plan</p>
                </div>
      ...
      <div className="bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-700">
        ...
        {transactions && transactions.length > 0 ? (
            ...
            <table className="min-w-full divide-y divide-gray-700">
              ...
              <tbody className="bg-gray-800 divide-y divide-gray-700">
                {transactions.map((transaction, index) => {
                  ...
                  const isPaid =
                    status === 'completed' || status === 'paid' || status === 'succeeded';
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-300 font-mono">
                          {transaction.invoiceId ||
                            `INV-${(transaction._id || '').substring(0, 8).toUpperCase()}`}
                        </span>
                      </td>
          ...
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-400">You don't have any transactions yet.</p>
            <div className="mt-6">
