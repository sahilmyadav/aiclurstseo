  ...
  const checkSubscriptionStatus = useCallback(
    async (userId, token) => {
      ...
      try {
        ...
        try {
          const response = await axios.get(`${API_BASE}/api/subscription/verify?userId=${userId}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          ...
          setSubscriptionError(
            error.response?.data?.error || error.message || 'Failed to check subscription status'
          );
          setSubscriptionData({
            active: false,
            planType: null,
  ...
  const fetchUserTransactions = useCallback(
    async (userId, token) => {
      ...
      try {
        ...
        const response = await axios.get(`${API_BASE}/api/subscription/transactions/${userId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
        ...
        setTransactionsError(
          error.response?.data?.message || error.message || 'Failed to fetch transaction history'
        );
        setTransactions([]);
      } finally {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        ...
        setPlansError(null);
        const { data } = await axios.get(`${API_BASE}/api/subscription/plans`);
        setPlans(data || []);
      } catch (error) {
        console.error('Error fetching plans:', error);
        setPlansError(error?.response?.data?.message || 'Failed to load subscription plans');
      } finally {
