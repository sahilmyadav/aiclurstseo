...
const Reviews = () => {
  ...
  const { reviews, selectedBusiness, businesses, loading, selectBusiness, tokenDetails } =
    useGoogleBusiness();
  ...
  const AutoReplyControls = ({
    ...
    <div
      className={`mx-8 my-4 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-sm border ${
    ...
    >
      <div className="px-2 py-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                <div className="relative inline-flex items-center">
                  <label className="flex items-center cursor-pointer">
                    <div className="relative">
                      ...
                      ></div>
                      <div
                        className={`absolute left-1 top-1 bg-white dark:bg-gray-700 w-6 h-6 rounded-full shadow-md transform transition-transform duration-300 ${
                    ...
                    <div className="ml-3">
                      ...
                      <p
                        className={`text-xs ${
                          theme === 'dark' ? 'text-gray-400' : 'text-blue-700/80'
                        }`}
                      >
                        {isAutoReplyMode
              ...
              {isAutoReplyMode && (
                ...
                >
                  <Clock
                    className={`h-4 w-4 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-500'}`}
                  />
                  <span
        ...
        >
            ...
            {!isAutoReplying && isAutoReplyMode && (
              <div
                className={`flex items-center text-xs px-3 py-1 rounded-full ${
                  theme === 'dark' ? 'text-blue-400 bg-blue-900/20' : 'text-blue-700 bg-blue-50'
                }`}
              >
          ...
          {isAutoReplyMode && (
            <div
              className={`mt-3 w-full rounded-full h-2 ${
  ...
  const repliedReviews = allDisplayReviews.filter(
    (review) => !!review.reply || review.reviewData?.reviewReply?.comment
  ).length;
  const pendingReviews = totalReviews - repliedReviews;
  const averageRating =
    allDisplayReviews.length > 0
      ? (
          allDisplayReviews.reduce((sum, review) => sum + review.rating, 0) /
  ...
  const renderBusinessDropdown = () => (
    <div className="px-4 sm:px-6 lg:px-8 mb-4">
      <BusinessProfileDropdown onSelect={handleBusinessSelect} className="w-full sm:w-64" />
    </div>
  );
  ...
  const generateAIReview = useCallback(async () => {
    ...
    try {
      const businessName =
        selectedBusiness?.title || selectedBusiness?.locationName || 'our business';
      const businessType =
  ...
  const processNextReview = useCallback(async () => {
    ...
    const eligibleReviews = displayReviews.filter(
      (review) => !review.reply && !review.reviewData?.reviewReply
    );
  ...
  const handleAutoReplyAll = useCallback(() => {
    if (isAutoReplying) {
      ...
      setCurrentReplyIndex(0);
      toast.info(
        `Starting auto-reply to ${eligibleReviews.length} reviews with ${autoReplyDelay} min delay...`
      );
      ...
      <React.Fragment key={review.id}>
        <tr
          className={`hover:bg-opacity-80 transition-colors duration-200 ${
            theme === 'dark' ? 'hover:bg-gray-700/40' : 'hover:bg-gray-50/80'
          }`}
        >
          <td className="px-6 py-4">
            <div className="flex flex-col space-y-5">
              ...
              {hasReply && (
                <div
                  className={`mt-4 ml-4 pl-4 border-l-2 ${
                    theme === 'dark' ? 'border-blue-900/40' : 'border-blue-200'
                  }`}
                >
                  <div
                    className={`bg-gradient-to-r p-4 rounded-xl shadow-sm ${
                      theme === 'dark'
                  ...
                  >
                    <div className="flex">
                      ...
                      <div className="ml-3 flex-1 min-w-0">
                        ...
                        <p
                          className={`mt-1.5 text-xs leading-relaxed whitespace-pre-line ${
                            theme === 'dark' ? 'text-blue-200' : 'text-blue-800'
                          }`}
                        >
                          {reply}
    ...
    >
      ...
      <div className="mx-4 sm:mx-6 lg:mx-8 mb-6 sm:mb-8">
        <div className="grid grid-cols-1 gap-3 sm:gap-4 md:gap-5 lg:gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {/* Total Reviews */}
          <div
            className={`rounded-xl shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md hover:-translate-y-1 ${
                  ...
                  >
                    <div className="flex items-center text-xs text-green-500 whitespace-nowrap overflow-hidden">
                      ...
                      <span className="truncate">
                        +{Math.floor(totalReviews * 0.15)}% this month
                      </span>
                    </div>
                  </div>
                )}
                  ...
                  <div className="flex items-center min-w-0">
                    ...
                    <div className="ml-3 min-w-0">
                      ...
                      <div className="flex items-center space-x-1.5">
                        ...
                        <div className="flex -space-x-0.5">{renderStars(averageRating)}</div>
                      </div>
                    </div>
                  </div>
                  <span
                ...
                >
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                    <div
                      ...
                      style={{
                        width: `${totalReviews > 0 ? (repliedReviews / totalReviews) * 100 : 0}%`,
                      }}
                    ></div>
                  </div>
                </div>
              ...
              <div className="flex flex-col">
                ...
                >
                  <div className="flex items-center justify-between text-xs">
                    ...
                    >
                      {totalReviews > 0
                        ? Math.round(((totalReviews - pendingReviews) / totalReviews) * 100)
                        : 0}
                      %
                    </span>
                  </div>
      ...
      <div
        className={`bg-gradient-to-br rounded-xl shadow-sm overflow-hidden mx-4 sm:mx-6 lg:mx-8 transition-all duration-300 hover:shadow-md ${
          theme === 'dark'
      ...
      >
          ...
          <table className="min-w-full">
            <thead
              className={`bg-gradient-to-r ${
                theme === 'dark' ? 'from-blue-900/40 to-blue-900/20' : 'from-blue-50 to-blue-100/80'
              }`}
            >
              <tr>
                <th
        ...
        <div
          className={`px-6 py-4 flex items-center justify-between border-t ${
            theme === 'dark' ? 'bg-gray-800/50 border-gray-700/30' : 'bg-white border-blue-100'
          }`}
        >
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className={`text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-blue-700/80'}`}>
                Showing <span className="font-medium text-blue-900">{displayReviews.length}</span>{' '}
                of <span className="font-medium text-blue-900">{reviews.length}</span> reviews
              </p>
            </div>
            <div>
              <nav
