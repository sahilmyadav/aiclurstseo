...
// Get BACKEND_URL dynamically (supports IP-based hosting)
const BACKEND_URL = getApiBaseUrl();

// Validate BACKEND_URL
if (!BACKEND_URL) {
  console.error('BACKEND_URL is not defined. Please check your configuration.');
...
const PostCard = ({ post, onEdit, onDelete, selectedBusiness }) => {
  ...
  return (
    <div
      className={`rounded-xl p-4 mb-4 shadow-lg hover:shadow-xl transition-all duration-300 ${
        theme === 'dark'
    ...
    >
      <div className="flex justify-between items-start mb-3">
        <div className="flex items-center gap-3">
          ...
          >
            <FaGoogle
              className={`text-lg ${theme === 'dark' ? 'text-blue-300' : 'text-blue-600'}`}
            />
          </div>
          <div>
      ...
      <div className="pl-1">
        <p
          className={`text-sm leading-relaxed mb-3 line-clamp-3 ${theme === 'dark' ? 'text-white/90' : 'text-gray-700'}`}
        >
          {post.content}
        </p>
        {post.media && (
          <div
            className={`mt-2 w-full h-36 rounded-lg flex items-center justify-center overflow-hidden ${
              theme === 'dark'
          ...
          >
            ...
            <span
              className={`text-xs px-3 py-1.5 rounded-full backdrop-blur-sm ${
                theme === 'dark' ? 'text-white/70 bg-black/40' : 'text-gray-600 bg-gray-200'
              }`}
            >
              View Media
            </span>
        ...
        <div
          className={`mt-3 pt-2 text-xs space-y-1.5 ${
            theme === 'dark'
          ...
          {post.status === 'scheduled' ? (
            <>
              <div className="flex items-center gap-1.5">
                <FaCalendarAlt
                  className={`flex-shrink-0 ${theme === 'dark' ? 'text-blue-400/80' : 'text-blue-500'}`}
                />
                <span>
              ...
              {post.isRecurring && (
                <div className="flex items-start gap-1.5">
                  <FaSync
                    className={`mt-0.5 flex-shrink-0 ${theme === 'dark' ? 'text-blue-400/80' : 'text-blue-500'}`}
                  />
                  <div>
              ...
              <div
                className={`flex items-center gap-1.5 ${theme === 'dark' ? 'text-blue-300/80' : 'text-blue-600'}`}
              >
            ...
            <div className="flex items-center gap-1.5">
              <FaCheckCircle
                className={`flex-shrink-0 ${theme === 'dark' ? 'text-green-400/80' : 'text-green-500'}`}
              />
            ...
            <div className="flex items-center gap-1.5">
              <FaEdit
                className={`flex-shrink-0 ${theme === 'dark' ? 'text-amber-400/80' : 'text-amber-500'}`}
              />
...
const Posts = () => {
  ...
  const fetchPosts = async (accountId, locationId, loadMore = false) => {
    ...
    try {
      ...
      const formattedPosts = postsList.map((post) => {
        ...
        if (
          statusFromApi === 'live' ||
          statusFromApi === 'posted' ||
          statusFromApi === 'processing'
        ) {
  ...
  const getAutoKeywords = () => {
    ...
    if (selectedBusiness.locationName) {
      ...
      businessName =
        typeof selectedBusiness.title === 'string'
          ? selectedBusiness.title
    ...
    if (selectedBusiness.primaryCategory) {
      categoryName =
        typeof selectedBusiness.primaryCategory === 'string'
          ? selectedBusiness.primaryCategory
          : selectedBusiness.primaryCategory.displayName ||
            selectedBusiness.primaryCategory.name ||
            '';
    } else if (selectedBusiness.categories?.primaryCategory) {
      const category = selectedBusiness.categories.primaryCategory;
      categoryName =
        typeof category === 'string' ? category : category.displayName || category.name || '';
  ...
  const handleSavePost = async (e) => {
    ...
    try {
      ...
      const postKeywords =
        currentPost.keywordsArray.length > 0 ? currentPost.keywordsArray.join(', ') : undefined;
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          errorMessage =
            'The requested resource was not found. Please check the business account and location.';
  ...
  return (
    <div
      className={`min-h-screen w-full flex ${theme === 'dark' ? 'text-white bg-transparent' : 'bg-[radial-gradient(at_40%_20%,hsl(250,91%,97%)_0px,transparent_50%),radial-gradient(at_80%_0%,hsl(340,82%,97%)_0px,transparent_50%),radial-gradient(at_0%_50%,hsl(160,84%,97%)_0px,transparent_50%)]'}`}
    >
      <SideNav />
      <div className="flex-1 transition-all duration-300 ease-in-out w-full overflow-x-hidden pl-0 md:pl-4">
        <div className="w-full max-w-full px-2 sm:px-4 md:px-6">
          <div className="mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex-shrink-0 flex flex-col sm:flex-row gap-3">
                ...
                {!isGoogleConnected && (
                  <p
                    className={`text-xs mt-1 text-center sm:text-right ${theme === 'dark' ? 'text-red-400' : 'text-red-600'}`}
                  >
                    <a
          ...
          {/* Tabs */}
          <div
            className={`border-b mb-6 backdrop-blur-sm rounded-t-lg ${
              theme === 'dark' ? 'border-white/10 bg-[#1a1b2e]/50' : 'border-gray-200 bg-white'
            }`}
          >
          ...
          <div>
            {!isGoogleConnected ? (
              <div className="text-center py-12">
                <div
                  className={`bg-gradient-to-r from-blue-600/20 to-indigo-600/20 border rounded-xl p-8 ${
                ...
                >
                  ...
                  >
                    Connect your Google My Business account to create, schedule, and manage posts
                    directly from here.
                  </p>
                  <a
                    href="/dashboard/integrations"
              ...
              <div className="col-span-full text-center py-12">
                ...
                >
                  <FaEdit
                    className={`text-2xl ${theme === 'dark' ? 'text-indigo-400' : 'text-indigo-600'}`}
                  />
                </div>
                <h3
                  className={`text-lg font-medium mb-1 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}
                >
                ...
                <form onSubmit={handleSavePost}>
                  <div className="mb-6">
                    <div className="flex justify-between items-center mb-2">
                      <label
                        className={`block text-sm font-medium ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}
                      >
                        Post Content
                      </label>{' '}
                      <button
                        ...
                        disabled={
                          isGeneratingAI ||
                          !selectedBusiness ||
                          currentPost.keywordsArray.length === 0
                        }
                          ...
                          <div className="flex gap-4">
                            <div className="flex-1 relative">
                              ...
                              <DatePicker
                                ...
                                onChange={(date) =>
                                  setCurrentPost({ ...currentPost, scheduledFor: date })
                                }
                                showTimeSelect
                                timeFormat="HH:mm"
                                timeIntervals={15}
                            ...
                            <div className="pl-6 space-y-3">
                              <div>
                                ...
                                <select
                                  ...
                                  onChange={(e) =>
                                    setCurrentPost({ ...currentPost, frequency: e.target.value })
                                  }
                              ...
                              <div>
                                ...
                                <div className="relative">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setCurrentPost({ ...currentPost, time: e.target.value })
                                    }
                                    className="w-full bg-[#1a1a2e] border border-white/10 rounded-lg pl-10 pr-3 py-2 text-white/90"
                                  />
                                </div>
                  ...
                  <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
                    ...
                    <button
                      ...
                      disabled={
                        isCreatingPost ||
                        (currentPost.keywordsArray.length === 0 && !currentPost.content)
                      }
